...
bot.command('whoisprakhar', (ctx) =>
  ctx.reply('Prakhar is a web defdfsdveloper and is going to get placement soon.')
);


bot.command('Integrtr', (ctx) =>
  ctx.reply('INTEGRTR is an enterprise technology startup that helps enterprises in their digital transformation journey to the cloud')
);

bot.command('placementstatus', (ctx) =>
  ctx.reply('Congratulations!!!🎉🎉🎉🎉🎉🎉. You are selected as an intern at INTEGRER')
);

bot.command('node', (ctx) =>
  ctx.reply('Mr Senthil is a good mentor. He taught us nodejs in semester 6.')
);
